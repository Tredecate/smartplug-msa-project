...
# App config
try:
    with open(Path("./default.config/default.healthcheck_app_conf.yml"), 'r') as f:
...
# Log config
try:
    with open(Path("./default.config/default.healthcheck_log_conf.yml"), 'r') as f:
...
# App config
try:
    with open(Path("/config/healthcheck_app_conf.yml"), 'r') as f:
...
# Log config
try:
    with open(Path("/config/healthcheck_log_conf.yml"), 'r') as f:
...
CHECKLIST_CONFIG = config_dict["services"]["checklist"]
